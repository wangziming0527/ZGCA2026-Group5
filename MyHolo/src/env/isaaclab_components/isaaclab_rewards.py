...
from holomotion.src.env.isaaclab_components.isaaclab_motion_tracking_command import RefMotionCommand
import torch
from isaaclab.envs import ManagerBasedRLEnv

def adaptive_action_rate_l2_per_dof_g1(
    env: ManagerBasedRLEnv,
    command_name: str = "ref_motion",
    ref_prefix: str = "ref_",
    # 下半身参数 (0-11) - 宽松：速度容忍度高，保底惩罚极低
    velocity_std_legs: float = 1.5,
    min_scale_legs: float = 0.8,
    # 上半身参数 (12-28) - 不衰减
    velocity_std_upper: float = 2.0,
    min_scale_upper: float = 1.0,
) -> torch.Tensor:
    """
    宇树 G1 (29 DoF) 专属的逐关节自适应动作平滑惩罚。
    将腿部与腰/臂/头部分开处理，下半身更宽松以利于动态追踪，上半身更严格以保持稳定。
    """
    # 1. 逐关节计算动作差值的平方 [B, 29]
    action_diff = env.action_manager.action - env.action_manager.prev_action
    dof_penalty_matrix = torch.square(action_diff)

    # 2. 获取参考动作中逐关节的角速度 [B, 29]
    ...
    # 3. 构建与 DoF 数量一致的张量参数
    num_dof = int(action_diff.shape[-1])
    device = action_diff.device
    
    # 初始化 Tensor
    vel_std = torch.ones(num_dof, device=device)
    min_scale = torch.ones(num_dof, device=device)
    
    # 赋值：下半身 (Left Leg 0-5, Right Leg 6-11)
    leg_end = min(12, num_dof)
    vel_std[0:leg_end] = velocity_std_legs
    min_scale[0:leg_end] = min_scale_legs

    # 赋值：上半身 (Waist/Arms/Head)
    if num_dof > 12:
        vel_std[12:num_dof] = velocity_std_upper
        min_scale[12:num_dof] = min_scale_upper

    # 4. 逐关节计算自适应权重矩阵 [B, 29]
    # 利用广播机制，每个关节除以自己的 vel_std
    decay_matrix = torch.exp(-torch.square(ref_dof_vel / vel_std))
    scale_matrix = min_scale + (1.0 - min_scale) * decay_matrix

    # 5. 先相乘，后求和 [B]
    adaptive_penalty = torch.sum(dof_penalty_matrix * scale_matrix, dim=1)
    
    return adaptive_penalty


def adaptive_action_rate_l2_for_tracking(
    env: ManagerBasedRLEnv,
    command_name: str = "ref_motion",  # motion_tracking中的command名字通常是这个
    ref_prefix: str = "ref_",
    velocity_std: float = 1.0,  # 动作库中速度普遍较大，std可以适当放宽到 1.0 或 1.5
    min_scale: float = 0.05,    # 推荐 0.05
) -> torch.Tensor:
    ...
    # 2. 获取参考动作(Reference Motion)的当前线速度作为评判标准
    command: RefMotionCommand = env.command_manager.get_term(command_name)
    # 提取参考动作根节点的世界系线速度 [B, 3]
    ref_root_lin_vel = command.get_ref_motion_root_global_lin_vel_cur(prefix=ref_prefix)
    
    # 计算参考动作的速率 (只取 xy 水平速度，或者取 xyz 整体速度，这里取 xyz)
    cmd_norm = torch.norm(ref_root_lin_vel, dim=1)

    # 3. 计算自适应衰减系数
    decay = torch.exp(-torch.square(cmd_norm / velocity_std))
    scale = min_scale + (1.0 - min_scale) * decay
