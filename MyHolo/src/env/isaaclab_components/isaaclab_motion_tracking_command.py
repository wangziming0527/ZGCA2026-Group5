...
class RefMotionCommand(CommandTerm):
    ...
    def _update_true_c_keys(self):
        """动态计算并存储基于当前绝对帧数的真实 Bin Keys (完美模拟 Mosaic)"""
        if not hasattr(self, "_motion_cache") or self._motion_cache is None:
            return

        dataset = self._motion_cache._datasets.get(self._motion_cache._mode)
        if dataset is None:
            return

        stride = getattr(dataset, "window_stride", dataset.max_frame_length)
        max_len = getattr(dataset, "max_frame_length", 500)

        # 获取环境最初被分配的原始 Clip Keys
        base_keys = self._motion_cache.motion_keys_for_indices(self._clip_indices)
        frame_idxs = self._frame_indices.cpu().numpy()

        for i, c_key in enumerate(base_keys):
            # 如果没有启用滑动窗口，或是验证集，直接回退
            if stride >= max_len or "__start_" not in c_key or "_len_" not in c_key:
                self.current_true_keys[i] = c_key
                continue

            # 解析原始 Key: e.g. "CMU_01_01__start_0_len_500"
            parts = c_key.split("__start_")
            raw_motion_key = "__start_".join(parts[:-1]) # 处理名字中自带__start_的边界情况
            win_start = int(parts[-1].split("_len_")[0])

            # 1. 计算当前时刻机器人的 绝对帧数
            current_abs_frame = win_start + frame_idxs[i]
            # 2. 计算当前时刻 真实所属的 Bin 起点
            true_bin_start = (current_abs_frame // stride) * stride

            # 3. 查表获取原动作总长度，以正确拼接后缀
            orig_len = 0
            if hasattr(dataset, "clips") and raw_motion_key in dataset.clips:
                orig_len = int(dataset.clips[raw_motion_key].get("length", 0))

            if orig_len > 0:
                window_length = min(max_len, orig_len - true_bin_start)
            else:
                window_length = max_len

            # 4. 拼装成真正的 Mosaic Bin Key
            true_c_key = f"{raw_motion_key}__start_{true_bin_start}_len_{window_length}"
            self.current_true_keys[i] = true_c_key
    ...
    def _init_motion_lib(self):
        ...
        if backend == "hdf5_simple":
            ...
            # [新增] 从配置读取 stride，默认回退为最大长度
            train_window_stride = int(mcfg.get("train_window_stride", max_frame_length))
            val_window_stride = int(mcfg.get("val_window_stride", max_frame_length))

            print(f'!!!train_window_stride:{train_window_stride}')
            ...
            train_dataset = Hdf5MotionDataset(
                ...
                window_stride=train_window_stride,  # [注入配置]
                handpicked_motion_names=mcfg.get("handpicked_motion_names", None),
                handpicked_motion_txt=mcfg.get("handpicked_motion_txt", None),
                # ======== [新增以下两行] ========
                handpicked_motion_json=mcfg.get("handpicked_motion_json", None),
                handpicked_motion_cluster_ids=mcfg.get("handpicked_motion_cluster_ids", None),
                # ===============================
            ...
            if val_roots is not None:
                ...
                val_dataset = Hdf5MotionDataset(
                    ...
                    max_frame_length=max_frame_length,
                    min_window_length=min_frame_length,
                    window_stride=val_window_stride,  # [注入配置]
                    world_frame_normalization=world_frame_norm,
                    allowed_prefixes=allowed_prefixes,
                    handpicked_motion_names=mcfg.get("handpicked_motion_names", None),
                    handpicked_motion_txt=mcfg.get("handpicked_motion_txt", None),
            ...
            # ======== [新增: 解析并传递 curriculum 配置] ========
            curriculum_node = mcfg.get("curriculum", OmegaConf.create({}))
            curriculum_cfg = OmegaConf.to_container(curriculum_node, resolve=True)
            # ===================================================
            ...
            self._motion_cache = MotionClipBatchCache(
                ...
                curriculum_cfg=curriculum_cfg,  # [新增]
            )
    ...
    def _init_per_env_cache(self):
        ...
        # [新增] 初始化状态与事件列表
        if not hasattr(self, "current_true_keys"):
            self.current_true_keys = [""] * self.num_envs
        if not hasattr(self, "event_buffer"):
            self.event_buffer = []
            
        self._update_true_c_keys()
    ...
    def _update_true_c_keys(self):
        ...
        for i, c_key in enumerate(base_keys):
            ...
            parts = c_key.split("__start_")
            raw_motion_key = "__start_".join(parts[:-1])
            win_start = int(parts[-1].split("_len_")[0])
            ...
            window_length = min(max_len, orig_len - true_bin_start) if orig_len > 0 else max_len
            self.current_true_keys[i] = f"{raw_motion_key}__start_{true_bin_start}_len_{window_length}"
    ...
    def reset(
        ...
        # --- [修复] 提前并安全地解析出 idxs 张量 ---
        if isinstance(env_ids, torch.Tensor):
            idxs = env_ids.to(self.device).long()
        elif isinstance(env_ids, slice):
            idxs = torch.arange(self.num_envs, device=self.device, dtype=torch.long)
        else:
        ...
        if self._env.common_step_counter > 0:
            ...
            # 1. 获取这批死亡环境最初被分配的原始 Clip Keys (真正的起跑线)
            # 注意：此时 _resample 还没执行，_clip_indices 仍然指向刚刚跑完的旧动作
            start_keys = self._motion_cache.motion_keys_for_indices(self._clip_indices[idxs])

            for i, env_id in enumerate(idxs.tolist()):
                start_c_key = start_keys[i]
                if start_c_key and "__start_" in start_c_key:
                    motion_key = start_c_key.split("__start_")[0]
                    
                    # 2. 获取该环境这局游戏的真实存活步数 (寿命)
                    progress_frames = int(self._env.episode_length_buf[env_id].item())
                    
                    # 3. 判断是否因为摔倒而失败
                    is_failure = 1.0 if self._env.reset_terminated[env_id].item() else 0.0
                    
                    # 4. 【最关键】必须追加 4 元组！激活 Sampler 的滑动窗口
                    self.event_buffer.append((motion_key, start_c_key, is_failure, progress_frames))
        # ======================================================
        
        # 使用安全的 idxs 更新状态
        self._motion_end_mask[idxs] = False
        self.motion_end_counter[idxs] = 0

        # Do not apply cache swap inside per-env reset; defer to PPO barrier.
        # Always resample only the requested envs here.
        self._resample_command(idxs, eval=self._is_evaluating)
    ...
    def apply_cache_swap_if_pending_barrier(self, global_rl_iter: int = None) -> bool:
        """Apply a pending cache swap at a rollout barrier.

        Returns:
            bool: True if a swap was applied, otherwise False.
        """
        # [新增] 透传真实的 PPO Iteration 给底层 DataLoader
        if global_rl_iter is not None:
            self._motion_cache.set_global_iteration(global_rl_iter)
        ...
        if hasattr(self, "event_buffer") and len(self.event_buffer) > 0:
            if self._motion_cache is not None:
                self._motion_cache.update_global_scores(self.event_buffer)
            self.event_buffer = []
        # ============================================
    ...
    def _update_command(self):
        ...
        # ======== [新增] ========
        # 在任何可能导致环境重置(截断)的逻辑之前，更新真实的 Bin Key。
        # 如果当前 step 环境摔倒了，重置(reset)发生在 compute() 之后。
        # 此时 PPO 去拿 Key，拿到的就是这里被冻结的“案发现场” Key！
        self._update_true_c_keys()
        # ========================
    ...
    def _resample_when_motion_end_cache(self):
        ...
        if torch.any(need_resample):
            ...
            # 在马上要覆盖 _clip_indices 之前，提取这些成功通关的动作信息
            success_keys = self._motion_cache.motion_keys_for_indices(self._clip_indices[resample_ids])
            success_frames = self._frame_indices[resample_ids].cpu().numpy()
            
            for i, env_id in enumerate(resample_ids.tolist()):
                start_c_key = success_keys[i]
                if start_c_key and "__start_" in start_c_key:
                    motion_key = start_c_key.split("__start_")[0]
                    # 因为是自然播放完毕没有摔倒，所以 is_failure 绝对是 0.0
                    self.event_buffer.append((motion_key, start_c_key, 0.0, int(success_frames[i])))
            # ===================================================         
    ...
    def get_current_motion_keys(
        ...
        # ======== [修改为返回真实的 Bin Key] ========
        if hasattr(self, "current_true_keys"):
            return self.current_true_keys
