...
def main(cfg: OmegaConf) -> None:
    ...
    # ==== 提取白名单配置并读取 txt ====
    handpicked_motion_txt = cfg.get("handpicked_motion_txt", None)
    whitelist_names = []
    if handpicked_motion_txt is not None:
        if not os.path.exists(handpicked_motion_txt):
            logger.warning(f"Whitelist txt file not found at {handpicked_motion_txt}. Proceeding without whitelist.")
        else:
            with open(handpicked_motion_txt, 'r', encoding='utf-8') as f:
                for line in f:
                    line = line.strip()
                    if line:  # 忽略空行
                        whitelist_names.append(line)
            logger.info(f"Loaded {len(whitelist_names)} whitelist rules from {handpicked_motion_txt}")
    # ==================================
        ...
        for dirpath, _, filenames in os.walk(base_dir):
            for fname in filenames:
                if fname.endswith(".npz"):
                    ...
                    if whitelist_names:
                        is_matched = False
                        for allowed_name in whitelist_names:
    ...
    if whitelist_names:
        logger.info(f"Filtered out {filtered_out_count} motions not in the whitelist.")
    ...
    assert len(motion_entries) > 0, "No motion NPZ files found (or all were filtered out)"
